...
gpu_manager = GPUManager(gpu_ids="0")
...
for i, (image_BGR, predicted_mask, true_mask, pred_points, true_points) in enumerate(
    ...
    axs[0].scatter(t_center[0] * w, t_center[1] * h, c='lightgreen', s=60, edgecolors='white', marker='X',
                   label='True Center')

    for j, (x, y) in enumerate(p_landmarks):
        axs[0].scatter(x * w, y * h, c='red', s=40, edgecolors='white', marker='o',
                       label='Pred Landmarks' if j == 0 else "")

    axs[0].scatter(p_center[0] * w, p_center[1] * h, c='lime', s=60, edgecolors='white', marker='X',
                   label='Pred Center')

    axs[0].legend(loc='upper right', fontsize='7')
    axs[0].set_xlim(0, w)
    axs[0].set_ylim(h, 0)
    axs[0].set_title("Landmarks: Pred vs True")
    axs[0].axis('off')
